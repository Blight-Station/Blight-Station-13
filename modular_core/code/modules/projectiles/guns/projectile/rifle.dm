/// Rifles go here
/// Specifically 2 Handed projectile Weapons with manually cycling actions
...
/obj/item/gun/projectile/contender/vintage //A different kind of Springfield.
	name = "single shot rifle"
	desc = "A very old pattern of rifle. Any markings it has have long since worn away or never existed in the first place.It chambered for vintage rounds."
	icon_state = "single"
	item_state = "single"
	icon_retracted = "single-open"
	origin_tech = list(TECH_COMBAT = 1, TECH_MATERIAL = 3)
	caliber = "vintage"
	slot_flags = SLOT_BACK
	load_method = SINGLE_CASING //Its not like you need a clip for this
	handle_casings = CYCLE_CASINGS
	is_wieldable = TRUE
	ammo_type = /obj/item/ammo_casing/vintage
	magazine_type = null

/obj/item/gun/projectile/contender/vintage/special_check(mob/user)
	if(retracted_bolt)
		to_chat(user, "<span class='warning'>You can't fire \the [src] while the trapdoor is open!</span>")
		return 0
	return ..()

/obj/item/gun/projectile/contender/vintage/attack_self(mob/user as mob)
	if(chambered)
		chambered.forceMove(get_turf(src))
		chambered = null
		var/obj/item/ammo_casing/C = loaded[1]
	...
	if(!retracted_bolt)
		to_chat(user, "<span class='notice'>You flip open the trapdoor on \the [src], allowing you to reload.</span>")
		playsound(user, 'sound/weapons/riflebolt.ogg', 60, 1)
		icon_state = icon_retracted
		item_state = icon_retracted
		retracted_bolt = 1
		...
		to_chat(user, "<span class='notice'>You close the trapdoor onto the bullet, allowing you to fire.</span>")

	else
		to_chat(user, "<span class='notice'>You close the trapdoor on the empty chamber, leaving the gun empty.</span>")
...
/obj/item/gun/projectile/contender/load_ammo(var/obj/item/A, mob/user)
	if(!retracted_bolt)
		to_chat(user, "<span class='notice'>You can't load \the [src] without opening the trapdoor.</span>")
		return
	..()

/obj/item/gun/projectile/contender/unload_ammo(mob/user, var/allow_dump=1)
	if(!retracted_bolt)
		to_chat(user, "<span class='notice'>You can't unload \the [src] without opening the trapdoor.</span>")
		return
	..()
